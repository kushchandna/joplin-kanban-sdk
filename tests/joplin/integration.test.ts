...
describe.skipIf(!ENABLED)('integration tests (requires running Joplin CLI)', () => {
  ...
  it('creates a board, manipulates it, and reads it back', async () => {
    ...
    const saved = await save_board(client, jboard, board, { syncAfterWrite: false });
    expect(saved.board.columns[0].cards).toHaveLength(2);

    const fetched = await fetch_board(client, jboard.noteId, { syncBeforeRead: false });
    ...
    const saved2 = await save_board(client, fetched, modified, { syncAfterWrite: false });
  ...
  it('converts inline card to external and back', async () => {
    ...
    const savedExternal = await save_board(client, saved, withExternal.board, { syncAfterWrite: false });
